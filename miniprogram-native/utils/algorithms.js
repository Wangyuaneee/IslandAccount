...
function runBfs(grid,delay,onStep,shouldPause){return new Promise(resolve=>{const rows=grid.length,cols=grid[0].length;const visited=createVisited(rows,cols);let islands=0;let steps=0;function enqueueBfs(sr,sc){const q=[[sr,sc]];visited[sr][sc]=true;function step(){if(q.length===0){resolveIsland();return}const rc=q.shift();const r=rc[0];const c=rc[1];steps++;onStep({stepNumber:steps,description:'BFS',currentPosition:{row:r,col:c}},clone(visited),islands);const ns=neighbors(r,c);for(let k=0;k<ns.length;k++){const nr=ns[k][0],nc=ns[k][1];if(inBounds(grid,nr,nc)&&grid[nr][nc]===1&&!visited[nr][nc]){visited[nr][nc]=true;q.push([nr,nc])}}schedule(step)}
function schedule(fn){if(shouldPause())return setTimeout(function(){schedule(fn)},100);setTimeout(fn,delay)}
...
next()})}
function runDfs(grid,delay,onStep,shouldPause){return new Promise(resolve=>{const rows=grid.length,cols=grid[0].length;const visited=createVisited(rows,cols);let islands=0;let steps=0;function dfs(sr,sc){const stack=[[sr,sc]];visited[sr][sc]=true;function step(){if(stack.length===0){resolveIsland();return}const rc=stack.pop();const r=rc[0];const c=rc[1];steps++;onStep({stepNumber:steps,description:'DFS',currentPosition:{row:r,col:c}},clone(visited),islands);const ns=neighbors(r,c);for(let k=0;k<ns.length;k++){const nr=ns[k][0],nc=ns[k][1];if(inBounds(grid,nr,nc)&&grid[nr][nc]===1&&!visited[nr][nc]){visited[nr][nc]=true;stack.push([nr,nc])}}schedule(step)}
function schedule(fn){if(shouldPause())return setTimeout(function(){schedule(fn)},100);setTimeout(fn,delay)}
function resolveIsland(){schedule(next)}
